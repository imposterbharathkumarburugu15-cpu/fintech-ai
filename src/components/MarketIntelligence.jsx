...
export function MarketIntelligence({ onOpenStock }) {
  ...
  <ChevronRight size={15} />
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-3">
        <StockColumn title={market === "watchlist" ? "Pinned instruments" : "Top gainers"} icon={<TrendingUp size={16}/>} stocks={filtered.slice(0, 3)} positive onOpenStock={onOpenStock} />
        <StockColumn title={market === "watchlist" ? "Recently viewed" : "Top losers"} icon={<TrendingDown size={16}/>} stocks={market === "watchlist" ? watchStocks.slice(1) : data.movers.filter(s => s[2].includes("−"))} onOpenStock={onOpenStock} />
        <StockColumn title="Most active" icon={<Zap size={16}/>} stocks={market === "watchlist" ? watchStocks.slice(0, 3) : data.movers.slice(1, 4)} positive active onOpenStock={onOpenStock} />
      </section>
...
function StockColumn({ title, icon, stocks, positive, active, onOpenStock }) { return <div className="market-card p-5"><div className="flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold"><span className={positive ? "text-emerald-400" : "text-rose-400"}>{icon}</span>{title}</h2><button className="text-[11px] text-zinc-500 hover:text-white">View all</button></div><div className="mt-4 space-y-2">{stocks.length ? stocks.map(([name, price, change, mark]) => <button onClick={() => onOpenStock?.(name)} className="stock-row w-full text-left" key={name}><span className="grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-br from-white/10 to-white/[.03] text-[10px] font-bold text-zinc-300">{mark}</span><div className="min-w-0 flex-1"><b>{name}</b><small>{active ? "High volume today" : price}</small></div><div className="text-right"><b>{active ? price : change}</b><small className={change.includes("−") ? "text-rose-400" : "text-emerald-400"}>{active ? change : "Today"}</small></div></button>) : <p className="py-5 text-center text-xs text-zinc-500">No matching instruments</p>}</div></div> }
...
function WatchCard({ stock, pinned, toggle, onOpenStock }) { const [name, price, change, mark] = stock; return <article onClick={() => onOpenStock?.(name)} className="market-card index-card cursor-pointer"><div className="flex items-center justify-between"><span className="grid h-8 w-8 place-items-center rounded-lg bg-white/[.07] text-xs font-bold">{mark}</span><button onClick={event => { event.stopPropagation(); toggle(); }} className={pinned ? "text-amber-300" : "text-zinc-600"}><Star size={17} fill={pinned ? "currentColor" : "none"}/></button></div><p className="mt-5 text-xs font-medium text-zinc-400">{name}</p><p className="mt-1 text-xl font-semibold">{price}</p><p className="mt-1 text-xs font-semibold text-emerald-400">{change}</p></article> }
