...
interface TopNavProps {
  activeView: string;
  onViewChange: (view: string) => void;  // This accepts string
  onToggleChat: () => void;
  user?: any;
}
 

export function TopNav({ activeView, onViewChange, onToggleChat, user }: TopNavProps) {
  ...
  // Safe way to get full name
  const fullName = user?.user_metadata?.full_name || user?.email || "User";
  
  // Safe way to get initials - with proper checks
  const initials = fullName
    .split(" ")
    .filter((s: string) => s && s.length > 0) // Filter out empty strings
    .map((s: string) => s[0]) // Get first character
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`px-4 pb-2 pt-3 text-[14px] font-medium transition-colors ${
                isActive
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="flex flex-col items-start hidden sm:flex">
                ...
                <span className="text-[10px] font-bold text-[#a855f7] leading-none mt-0.5 bg-purple-500/10 px-1.5 py-0.5 rounded-sm">
                  Pro Plan
                </span>
              </div>
            </button>
